...
- Look through the sources and replace `datalad_debian` with
  `datalad_<newname>` (hint: `git grep datalad_debian` should find all
  spots).
- Delete the example command implementation in `datalad_debian/__init__.py`
  by (re)moving the `HelloWorld` class.
- Implement a new command, and adjust the `command_suite` in
  `datalad_debian/__init__.py` to point to it.
- Replace `hello_cmd` with the name of the new command in
  `datalad_debian/tests/test_register.py` to automatically test whether the
